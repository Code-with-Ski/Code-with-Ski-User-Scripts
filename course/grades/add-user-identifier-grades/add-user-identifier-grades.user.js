// ==UserScript==
// @name         Add User Identifier - Gradebook
// @namespace    https://github.com/Code-with-Ski/Code-with-Ski-User-Scripts/course/grades/add-user-identifier-grades
// @version      1.0.0
...
(() => {
  ...
  if (
    isApprovedUser &&
    /^\/courses\/[0-9]+\/gradebook/.test(window.location.pathname)
  ) {
    watchForStudentSearchInput();
  }

  function watchForStudentSearchInput() {
    const studentSearchInput = document.querySelector(
      "input#student-names-filter"
    );
    if (studentSearchInput) {
      ...
      const observer = new MutationObserver((mutations) => {
        const addedStudentSearchInput = document.querySelector(
          "input#student-names-filter"
        );
